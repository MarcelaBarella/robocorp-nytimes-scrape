...
def main():
    ...
    def calculate_extra_columns(article):
        article["search_phrase_count"] = count_search_phrases(
...
def count_search_phrases(article, search_phrase):
    return (article["title"].count(search_phrase)
            + article["description"].count(search_phrase))
...
def contains_amount_of_money(article):
    ...
    if re.search(r'\$(?:[0-9]{1,3})(?:,[0-9]{3})*(?:\.[0-9]{1,2})?',
                 title_and_description) is not None:
        return True

    # Format 11 dollars | 11 USD
    if re.search(r'[0-9]+ (?:dollars|dollar|USD)',
                 title_and_description) is not None:
        return True
