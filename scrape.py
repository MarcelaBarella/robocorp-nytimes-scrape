...
def main():
    ...
    try:
        ...
        # Loads enough pages as to guarantee we get all data for the considered months
        while datetime.strptime(search_results.articles[-1]["date"], "%Y-%m-%d").date() > cut_date:
            if not search_results.load_more_articles():
                break

        # Filters only articles from the cut date onwards
        articles = list(filter(lambda article: datetime.strptime(article["date"], "%Y-%m-%d").date() >= cut_date, search_results.articles))
...
# Call the scrape() function, checking that we are running as a stand-alone script:
