...
def main():
    ...
    def download_image(article):
        filename_from_url = unquote(urlparse(article["picture_url"]).path.split("/")[-1])
        (filename, _) = urllib.request.urlretrieve(article["picture_url"], f"./output/{filename_from_url}")
